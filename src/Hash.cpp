 ...
 * 
 * @details 
 * This file contains the implementation of the Hash class, which provides functionality for hashing data using the SHA-256 algorithm. 
 * It includes methods for performing various operations required for the SHA-256 algorithm, such as message scheduling, choice, majority, and rounds. 
 * The class also provides methods for setting the data and salt values, computing the hash, and retrieving the computed hash value.
 * 
...
void Hash::message_schedule(uint32_t (&W)[64], uint8_t (&block)[64]) {
	...
	for (int i = 16; i < 64; i++) {
		W[i] = little_sigma_1(W[i - 2]) + W[i - 7] + little_sigma_0(W[i - 15]) +
			   W[i - 16];
	}
}
...
void Hash::round(uint32_t (&H)[8],
				 uint32_t round_constant,
				 uint32_t schedule_word) {
	uint32_t T1 = H[h] + big_sigma_1(H[e]) + choice(H[e], H[f], H[g]) +
...
void Hash::sha256() {
	...
	for (int i = 0; i < 8; ++i) {
		message.push_back(
			((((unsigned long long)l) >> (56 - (8 * i))) & 0xFFu));
	}
...
Hash::Hash(std::string data, std::string salt)
	: data(data), salt(salt), hash("") {
	sha256();
}

Hash::Hash(const Hash &hashObj)
	: data(hashObj.data), salt(hashObj.salt), hash(hashObj.hash) {}
